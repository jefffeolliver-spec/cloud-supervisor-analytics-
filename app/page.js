...
import { LoginScreen, IAPanel, RankingTab, ConfigTab, HistoricoTab, FormularioManual, parseFile, calcScore, tier, initials, avg, C, NAV, SYSTEM_PROMPT, supabase, StrategyAdvisor, ExportTab } from "./components/Helpers";
...
function Dashboard({user,onLogout}){
  ...
  async function handleFile(file){
    ...
    try{
      ...
      if(!uniqueColabs.length){setImportMsg("Nenhum colaborador encontrado.");return;}
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:20}}>
          {tab==="overview"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {(()=>{
                ...
                return(<div style={{marginBottom:4}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><div style={{width:7,height:7,borderRadius:"50%",background:"#059669"}}/><span style={{fontSize:11,color:"#888",letterSpacing:0.5}}>Operacao ao vivo · {equipes}</span></div><div style={{fontSize:20,fontWeight:800,letterSpacing:-0.5,color:"#111"}}>{saud}, {nomeExib}.</div></div>);
              })()}
                  ...
                  <div style={{background:"#fff",border:"1px solid #E5E5E5",borderRadius:12,overflow:"hidden"}}>
                    ...
                    <div style={{padding:"16px 14px"}}>
                      {[...data].sort((a,b)=>calcScore(b)-calcScore(a)).map((r,i)=>{
                        ...
                        return(<div key={i} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:11,fontWeight:600,color:"#333"}}>{r.nome.split(" ")[0]} {r.nome.split(" ").slice(-1)[0]}</span><span style={{fontSize:11,fontWeight:800,color:col}}>{sc}/100</span></div><div style={{height:8,background:"#F1F5F9",borderRadius:4,position:"relative"}}><div style={{width:`${Math.min(sc,100)}%`,height:"100%",background:col,borderRadius:4,transition:"width 0.5s"}}/><div style={{position:"absolute",top:0,left:"80%",width:2,height:"100%",background:"#94A3B8",borderRadius:1}}/></div></div>);
                      })}
                    </div>
                  </div>
                  <div style={{background:"#fff",border:"1px solid #E5E5E5",borderRadius:12,overflow:"hidden"}}>
                    <div style={{padding:"12px 16px",borderBottom:"1px solid #F0F0F0"}}><div style={{fontSize:13,fontWeight:700,color:"#111"}}>Distribuicao por Status</div></div>
                    <div style={{padding:"16px 14px"}}>
                      {(()=>{
                        ...
                        const items=[{l:"Top",v:top,col:"#059669",bg:"#D1FAE5",pct:Math.round(top/total*100)},{l:"Regular",v:reg,col:"#2563EB",bg:"#DBEAFE",pct:Math.round(reg/total*100)},{l:"Atencao",v:atc,col:"#D97706",bg:"#FEF3C7",pct:Math.round(atc/total*100)},{l:"Critico",v:cri,col:"#DC2626",bg:"#FEE2E2",pct:Math.round(cri/total*100)}];
                        return(<div><div style={{display:"flex",height:20,borderRadius:10,overflow:"hidden",marginBottom:16,gap:2}}>{items.filter(x=>x.pct>0).map((x,i)=>(<div key={i} style={{flex:x.pct,background:x.col,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:9,fontWeight:800,color:"#fff"}}>{x.pct>10?x.pct+"%":""}</span></div>))}</div><div style={{display:"flex",gap:8}}>{items.map((x,i)=>(<div key={i} style={{flex:1,background:x.bg,borderRadius:8,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:22,fontWeight:900,color:x.col}}>{x.v}</div><div style={{fontSize:10,color:x.col,fontWeight:700,marginTop:2}}>{x.l}</div><div style={{fontSize:10,color:"#888"}}>{x.pct}%</div></div>))}</div></div>);
                      })()}
                    </div>
                  </div>
                  <div style={{background:"#fff",border:"1px solid #E5E5E5",borderRadius:12,overflow:"hidden"}}>
                    <div style={{padding:"12px 16px",borderBottom:"1px solid #F0F0F0",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:13,fontWeight:700,color:"#111"}}>CPC vs Meta</div><div style={{fontSize:10,color:"#aaa"}}>meta diaria: {20*(datasSel.length||1)}</div></div>
                    <div style={{padding:"16px 14px"}}>
                      {[...data].sort((a,b)=>calcScore(b)-calcScore(a)).map((r,i)=>{
                        ...
                        return(<div key={i} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:11,fontWeight:600,color:"#333"}}>{r.nome.split(" ")[0]} {r.nome.split(" ").slice(-1)[0]}</span><span style={{fontSize:11,fontWeight:800,color:col}}>{r.cpc}/{meta} <span style={{fontSize:9,color:"#aaa"}}>({pct}%)</span></span></div><div style={{height:8,background:"#F1F5F9",borderRadius:4}}><div style={{width:`${Math.min(pct,100)}%`,height:"100%",background:col,borderRadius:4,transition:"width 0.5s"}}/></div></div>);
                      })}
                    </div>
                  </div>
                </>
          ...
          {tab==="export"&&<ExportTab datas={datas} supabase={supabase} config={config} data={data} datasSel={datasSel}/>}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:12}}>
                  ...
                  <div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:10,padding:14}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                      {["nome","equipe","supervisor","data","chamadas_recebidas","chamadas_realizadas","cpc","retidos","conversoes","eficiencia","tempo_produtivo","tempo_logado","localizacao"].map(c=>(<span key={c} style={{background:C.bgAlt,border:"1px solid "+C.border,borderRadius:4,padding:"2px 8px",fontSize:10,color:C.txtSub,fontFamily:"monospace"}}>{c}</span>))}
                    </div>
                    <div style={{fontSize:11,color:C.txtMuted,marginTop:8}}>Data no formato YYYY-MM-DD.</div>
                  </div>
                </div>
              )}
          ...
          <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"#fff",borderRadius:16,padding:0,zIndex:201,width:"min(340px,90vw)",boxShadow:"0 20px 60px rgba(0,0,0,0.3)",overflow:"hidden",fontFamily:"'Inter',system-ui,sans-serif"}}>
            ...
            <div style={{padding:"8px 0",maxHeight:300,overflowY:"auto"}}>
              {datas.map(d=>{
                ...
                return(<div key={d} onClick={()=>{const next=sel?tempSel.filter(x=>x!==d):[...tempSel,d];setTempSel(next);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 20px",cursor:"pointer",background:sel?"#EEF2FF":"#fff",borderBottom:"1px solid #F8F8F8"}}><div style={{width:18,height:18,borderRadius:4,border:"2px solid "+(sel?"#6366F1":"#D1D5DB"),background:sel?"#6366F1":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{sel&&<span style={{color:"#fff",fontSize:11,fontWeight:900}}>✓</span>}</div><span style={{fontSize:13,fontWeight:sel?700:400,color:sel?"#6366F1":"#333"}}>📅 {d}</span></div>);
              })}
            </div>
            <div style={{padding:"12px 20px",borderTop:"1px solid #E2E8F0",display:"flex",gap:8}}>
              <button onClick={()=>{if(tempSel.length===datas.length)setTempSel([]);else setTempSel([...datas]);}} style={{flex:1,background:"#F1F5F9",border:"none",borderRadius:8,padding:"9px 0",fontSize:12,fontWeight:600,color:"#475569",cursor:"pointer",fontFamily:"inherit"}}>{tempSel.length===datas.length?"Desmarcar todas":"Marcar todas"}</button>
              <button onClick={()=>{if(tempSel.length===0)return;setDatasSel(tempSel);loadData(tempSel);setDateModal(false);}} style={{flex:1,background:"#6366F1",border:"none",borderRadius:8,padding:"9px 0",fontSize:12,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Aplicar ({tempSel.length})</button>
            </div>
          </div>
        </>
      )}
